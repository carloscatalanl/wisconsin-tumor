...
mat=Train;    
mat(:,1)=[];  %   Eliminamos la primera columna (ID)


%   Separamos las clases
map_benigno=mat(:,10)==2; %   Encontramos todas los renglones ==2
benigno=mat(map_benigno,:);   %   Creamos la matriz limpia

map_maligno=mat(:,10)==4;
...
freqBenigno(:,11)=[];       %   Eliminamos las etiquetas
freqMaligno(:,11)=[];

%% 5.- Test (Calcular Test)

test=Test;
test(:,1)=[];   %   Eliminamos el ID de frecuencia 
test(:,10)=[];  %   Eliminamos la etiqueta 
